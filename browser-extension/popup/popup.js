...
async function saveToCloud(pdf) {
  try {
    ...
    if (selectedSource) {
      ...
      console.log('报告来源:', selectedSource);
    } else {
      // 如果未选择，尝试从 URL 自动识别
      const autoSource = detectSourceFromUrl(pdf.url);
      if (autoSource) {
        formData.append('source', autoSource);
        console.log('自动识别来源:', autoSource);
      }
    }
...
function detectSourceFromUrl(url) {
  ...
  for (const [domain, source] of Object.entries(sourceMap)) {
    if (urlLower.includes(domain)) {
      console.log(`自动识别来源: ${domain} -> ${source}`);
      return source;
    }
  }
  return '';
