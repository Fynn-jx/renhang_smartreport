...
【输出格式：Markdown格式】 必须严格遵守以下排版规则：

1. 直接输出翻译后的中文内容，保持原有段落结构。
...
{content}"""
    ...
    # 分批翻译时的简化提示词
    CHUNK_TRANSLATION_PROMPT = """请将以下文本翻译为简体中文。直接输出翻译后的中文内容，保持原有段落结构。
